...
RUNNER_VERSION = "product-free-rag-a6-one-shot-runner-v2"
EXPECTED_MANIFEST_SCHEMA = "product-free-rag-a6-freeze-manifest-v2"
EXPECTED_EVALUATION_ROLE = (
    "product_free_rag_a6_human_reviewed_first_one_shot"
)
HARDCODED_RUNTIME_PROFILE = {
...
def verify_freeze_manifest(
    ...
    *,
    ...
    if manifest.get("manifest_schema_version") != EXPECTED_MANIFEST_SCHEMA:
        raise RuntimeError("unexpected Product A6 freeze manifest schema")
    if manifest.get("status") != "human_reviewed_frozen_ready_for_exactly_one_execution":
        raise RuntimeError("Product A6 freeze manifest is not execution-ready")
    ...
    if not (
        ...
        raise RuntimeError("Product A6 freeze permissions are invalid")
    if manifest.get("audit", {}).get("gate_pass") is not True:
        raise RuntimeError("Product A6 frozen-row audit is not a clean pass")
    if manifest.get("human_review", {}).get("approved_rows") != 32:
        raise RuntimeError("Product A6 human review is not 32/32")
    if manifest.get("model") != {
        "tag": MODEL_TAG,
        "blob_sha256": MODEL_BLOB_SHA256,
        "num_ctx": 8192,
    }:
        raise RuntimeError("Product A6 model seal is invalid")
    ...
    if not (
        ...
        raise RuntimeError("Product A6 pipeline seal is invalid")
    ...
    for item in frozen_hashes:
        ...
        if not relative_path or relative_path in sealed_paths:
            raise RuntimeError("Product A6 frozen hashes contain a missing or duplicate path")
    ...
    if missing_paths:
        raise RuntimeError(f"Product A6 freeze manifest omitted sealed inputs: {missing_paths}")
    bound_inputs = {
    ...
    for label, item in bound_inputs.items():
        ...
        if (
            ...
            raise RuntimeError(f"Product A6 {label} is not included in the sealed inputs")

    frozen_set = manifest.get("frozen_set") or {}
    if frozen_set.get("row_count") != 32:
        raise RuntimeError("Product A6 frozen set row count is not 32")
    frozen_path = _resolve(root, Path(frozen_set["path"]))
    try:
        _relative(root, frozen_path)
    except ValueError as exc:
        raise RuntimeError("Product A6 frozen set escapes repository") from exc
    if sha256_path(frozen_path) != frozen_set.get("sha256"):
        raise RuntimeError("frozen A6 set SHA mismatch")
    return frozen_path
...
def run_regression_preflight(root: Path) -> dict[str, Any]:
    command = [
        ...
        "tests/v3/test_freeze_product_free_rag_a6.py",
        "tests/v3/test_score_product_free_rag_a6.py",
        "tests/v3/test_run_product_free_rag_a6_one_shot.py",
        "tests/v3/test_adjudicate_product_free_rag_a6.py",
...
def _exclusive_run_lock(path: Path):
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError("another Product A6 one-shot process is active") from exc
        try:
            yield
        finally:
            handle.seek(0)
...
def _run_one_shot_locked(
    *,
    ...
    result_stem = f"product_free_rag_a6_one_shot_{manifest_sha256}"
    ...
    if marker_path.exists():
        if not resume:
            raise RuntimeError(
                "Product A6 one-shot was already started; use --resume only for an interrupted run"
            )
        records = list(read_jsonl(output_path)) if output_path.exists() else []
        if any(row.get("type") == "summary" for row in records):
            raise RuntimeError("Product A6 results are already open; rerun is forbidden")
    else:
        if resume:
    ...
    if any(
        ...
        raise RuntimeError("frozen A6 row permissions or role are invalid")
    ...
    summary = {
        **summarize(
        ...
        "experimental_profile": {
            "identity_shortlist": HARDCODED_RUNTIME_PROFILE[
                "use_identity_shortlist"
            ],
            "compact_evidence_pack": HARDCODED_RUNTIME_PROFILE[
                "use_compact_evidence_pack"
            ],
            "device": rag.device,
        },
...
def run_one_shot(
    *,
    ...
    lock_path = (
        root
        / "data/v3/evaluation"
        / f"product_free_rag_a6_one_shot_{manifest_sha256}.lock"
    )
...
def dry_run_profile(*, device: str | None) -> dict[str, Any]:
    """Describe the sealed runtime profile without opening an A6 attempt."""
    return {
        ...
        "hardcoded_runtime_profile": dict(HARDCODED_RUNTIME_PROFILE),
        "one_shot_safety": {
            "maximum_execution_attempts": 1,
            "rerun_after_results_opened": False,
            "attempt_marker": True,
            "durable_per_slot_journal": True,
            "os_exclusive_lock": True,
        },
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the frozen Product Free RAG A6 set exactly once"
    )
...
def main() -> None:
    ...
    if args.dry_run:
        print(
            json.dumps(
                dry_run_profile(device=args.device),
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    if args.freeze_manifest is None:
        raise RuntimeError("--freeze-manifest is required unless --dry-run is used")
    print(
        json.dumps(
            run_one_shot(
                root=args.root,

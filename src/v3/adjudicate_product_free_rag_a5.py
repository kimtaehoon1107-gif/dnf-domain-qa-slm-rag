...
ADJUDICATOR_VERSION = "product-free-rag-a5-human-adjudicator-v1"
...
def _load_frozen(path: Path) -> list[dict[str, Any]]:
    rows = list(read_jsonl(path))
    if len(rows) != 32:
        raise RuntimeError("frozen A5 set must contain 32 rows")
...
def finalize_review(
    *,
    ...
    final = {
        "report_schema_version": "product-free-rag-a5-final-adjudication-v1",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Human-adjudicate Product Free RAG A5")

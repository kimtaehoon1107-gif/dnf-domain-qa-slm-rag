...
FREEZER_VERSION = "product-free-rag-a5-freezer-v2"
MANIFEST_SCHEMA_VERSION = "product-free-rag-a5-freeze-manifest-v2"
EXPECTED_CANDIDATE_SCHEMA = "product-free-rag-a5-candidate-v2"
EXPECTED_EVALUATION_ROLE = (
    "product_a5_unexecuted_candidate_pending_human_review"
)
...
DEFAULT_CANDIDATES = Path(
    "data/v3/evaluation/product_free_rag_a5_unexecuted32_draft.jsonl"
)
DEFAULT_PREFREEZE_MANIFEST = Path(
    "reports/v3/product_free_rag_a5_unexecuted32_manifest.json"
)
DEFAULT_VALIDATION = Path(
    "reports/v3/product_free_rag_a5_unexecuted32_validation.json"
)
DEFAULT_CHUNKS = Path(
    "data/v3/chunks/"
...
FROZEN_CODE_PATHS = (
    ...
    Path("src/v3/adjudicate_product_free_rag_a5.py"),
    ...
    Path("src/v3/run_product_free_rag_a5_one_shot.py"),
    Path("src/v3/schemas.py"),
    Path("src/v3/score_product_free_rag_a5.py"),
    ...
    Path("src/v3/freeze_product_free_rag_a5.py"),
    Path("tests/v3/test_freeze_product_free_rag_a5.py"),
    Path("tests/v3/test_adjudicate_product_free_rag_a5.py"),
    ...
    Path("tests/v3/test_run_product_free_rag_a5_one_shot.py"),
    Path("tests/v3/test_score_product_free_rag_a5.py"),
)
...
def verify_model() -> None:
    ...
    if match is None or match.group(1) != MODEL_BLOB_SHA256:
        raise RuntimeError("Ollama model blob differs from the Product A5 seal")
    if "PARAMETER num_ctx 8192" not in completed.stdout:
        raise RuntimeError("Product A5 requires Ollama num_ctx 8192")
...
def audit_frozen_rows(
    ...
    *,
    ...
    gates = {
        ...
        "requirements_54": sum(len(row["requirements"]) for row in rows) == 54,
        "evidence_units_47": evidence_count == 47,
        "evidence_coordinates_exact": not coordinate_failures,
        "unsupported_slots_exact": unsupported_slots == [4, 12, 20, 32],
        "metadata_answers_3": sum(
            row["expected_query_mode"] == "metadata"
            and row["expected_response_mode"] != "clarification"
            for row in rows
        )
        == 3,
        "clarification_1": coverage_counts["clarification"] == 1,
        "server_tables_2": sum(
...
def freeze_a5(
    *,
    ...
    validation_path = _resolve(root, validation_path)
    ...
    if _relative(root, chunks_path) != DEFAULT_CHUNKS.as_posix():
        raise RuntimeError("Product A5 freeze requires the canonical chunk corpus")
    if _relative(root, runtime_snapshot_path) != DEFAULT_RUNTIME_SNAPSHOT.as_posix():
        raise RuntimeError("Product A5 freeze requires the canonical runtime snapshot")
    ...
    frozen_path = output_dir / f"product_free_rag_a5_frozen_{frozen_sha256}.jsonl"
    manifest = {
        ...
        "prefreeze": {
            ...
            "validation": {
                "path": _relative(root, validation_path),
    ...
    manifest_path = output_dir / f"product_free_rag_a5_freeze_manifest_{manifest_sha256}.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze the independently reviewed Product Free RAG A5 set"
    )

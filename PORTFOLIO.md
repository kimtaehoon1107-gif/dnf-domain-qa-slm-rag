...
| cross-parent 값 일치 시 통과 허용 | 이 가드가 막은 claim 3건 재판정 시 정밀도 **66.7%**(실제 오답 2건 차단, 정답 오탐 1건). 대체용 BGE 의미 유사도 임계값도 승인 claim 142건 전수에서 실패(정상 claim 최저점 0.00002566 < 오류 claim 최저점 0.00005059, 임계값 부재) | 가드 유지 — 완화 시 slot 1류(다른 결제수단 값 오용) 오답 재발 확인됨 |
...
아홉째, 조건·주어 결속 오류(A6-7 등)의 원인 일부는 모델이 아니라 근거 팩 구성 방식일 수 있다. slot 2(점검 시각)를 원문 그대로 하나의 근거로 주면 정답을, 실제 서비스처럼 여러 조각(E1~E3)으로 쪼개 주면 봉인 결과와 같은 오답("게시 시각"을 "점검 시작 시각"으로 착각)을 그대로 재현했다. A6-7도 근거를 하나의 문맥으로 합치자 프롬프트나 모델의 thinking 모드 없이 정답이 나왔고, 지연시간은 thinking 모드 대비 약 4분의 1이었다. 이 결과는 [근거 문맥 파편화 진단](reports/v3/product_free_rag_evidence_context_fragmentation_diagnostic_20260816.json)에 있으며, 아직 `product_free_rag.py`의 근거 팩 구성 로직에는 반영하지 않았다. 반영과 재검증은 별도 라운드로 남긴다.

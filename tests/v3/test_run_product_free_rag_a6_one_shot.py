...
import src.v3.run_product_free_rag_a6_one_shot as runner
...
def test_dry_run_reports_the_a5_hardcoded_profile_and_one_shot_safety() -> None:
    result = runner.dry_run_profile(device="cuda")

    assert result["status"] == "dry_run_only_no_execution"
    assert result["experimental_profile"] == {
        "identity_shortlist": True,
        "compact_evidence_pack": True,
        "device": "cuda",
    }
    assert result["hardcoded_runtime_profile"] == {
...
def test_one_shot_lock_rejects_a_concurrent_process(tmp_path: Path) -> None:
    ...
    with runner._exclusive_run_lock(lock_path):
        with pytest.raises(RuntimeError, match="another Product A6"):
            with runner._exclusive_run_lock(lock_path):
                pass
...
def test_resume_records_indeterminate_slot_without_calling_it_again(
    ...
    stem = f"product_free_rag_a6_one_shot_{manifest_sha256}"
